...
async def schedule_image_post(
    *, user_id: str, image_post_id: UUID, apply_schedule=None
    ...
    # Already published — do not hit Ayrshare again (Modal replay of
    # finish_image_post after a successful /post would otherwise
    # create a second social post).
    ...
    try:
        ...
        # Same contract as video: Ayrshare treats a past scheduleDate as
        # "publish immediately". datetime.now() is already in the past by
        # the time the request arrives, so image posts were going live
        # the moment they left QA — ignoring the niche's posting windows.
