...
def check_publisher() -> CheckResult:
    """Social publishing. An unset key here means finished videos pile up
    with nowhere to go — the failure surfaces at the very last stage of a
    job, after every cent of generation is already spent."""
    from . import publisher  # deferred: keeps preflight import-order-safe

    if publisher.enabled():
        return _ok("publishing", "Zernio configured for social publishing.")
    ...
    # backend). A deploy that only generates and never publishes is a
    # valid, if unusual, configuration.
    return _warn(
        "publishing",
        "MARKETER_ZERNIO_API_KEY is not set — jobs will spend their full "
        "generation cost and then fail at the publishing stage.",
    )
...
def check_publisher_webhook() -> CheckResult:
    ...
    its optimistic status forever — as does an account whose token died."""
