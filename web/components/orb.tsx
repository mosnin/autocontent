...
// represents it.
//
// The shader is the Originkit "Cosmic Orb" source, used as delivered. Two
// modifications were made and both are marked `ADDED` inline:
//
//   1. `prefers-reduced-motion` support. The orb animates continuously; a
//      viewer who has asked the OS for less motion gets a single rendered
...
//
// See docs/ORB.md for the full source and usage guidance.
"use client";
...
// ADDED: browsers cap live WebGL contexts and evict the oldest silently.
// Stay well under the common ~16 so the app's own chrome (charts) keeps
// its contexts too.
...
function __OriginkitBase_Orb(props: OrbProps) {
    ...
    // ADDED: when WebGL is unavailable, over budget, or the context is
    // lost for good, fall back to a static gradient rather than a hole.
    ...
    useEffect(() => {
        ...
        // ADDED: context budget. Exceeding the browser cap silently kills
        // the OLDEST contexts, so orbs at the top of a long page would go
        // blank with no error. Take a slot or render the standin.
        ...
        // ADDED: honour the OS reduced-motion setting. The orb otherwise
        // animates forever on every page it appears on.
        const motionQuery =
        ...
        // ADDED: reduced motion renders one frame and stops. A single
        // still orb still reads as the mark; a looping one is the thing
        ...
        return () => {
            ...
            // ADDED: release the slot AND the GPU context. Without the
            // explicit loseContext, a route change leaks the context and
            // the budget drains as the user navigates.
            liveContexts = Math.max(0, liveContexts - 1);
            gl.getExtension("WEBGL_lose_context")?.loseContext();
        };
    }, []);

    const colorA = palette?.colorA ?? DEFAULTS.palette.colorA;
    const colorB = palette?.colorB ?? DEFAULTS.palette.colorB;
        ...
        >
            <div
                ...
                style={{
                    ...
                    borderRadius: "50%",
                    overflow: "hidden",
                    // ADDED: the standin. Same silhouette and palette, so a
                    // fallback orb still reads as the mark.
                    background: fallback
                        ? `radial-gradient(circle at 35% 30%, ${colorA}, ${colorB} 55%, ${background} 100%)`
                        : undefined,
                }}
            >
                {fallback ? null : (
...
/**
 * The agent mark at a fixed size, decorative by default.
 *
 * Use this rather than `Orb` anywhere the orb sits beside a label: it is
 * `aria-hidden` (the adjacent text already names the thing) and it does
 * not carry the 320px default size into a 24px slot.
 */
export function AgentMark({
    ...
    size?: number;
    className?: string;
    /** Supply only when the orb stands alone with no adjacent text. */
    label?: string;
}) {
    return (
        <span
        ...
        >
            <Orb size={size} style={{ width: size, height: size }} />
        </span>
    );
}
