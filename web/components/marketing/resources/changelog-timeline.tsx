...
export function ChangelogTimeline({ entries }: { entries: ChangelogEntry[] }) {
  return (
    <div className="relative">
      <span
        aria-hidden
        className="absolute inset-y-3 left-[0.4375rem] w-px bg-muted"
      />
      <Stagger className="space-y-10" gap={0.06}>
        {entries.map((entry) => (
          <div className="relative pl-10" key={entry.title}>
            <span
              aria-hidden
              className="absolute left-0 top-2 size-[0.9375rem] rounded-full border-[3px] border-white bg-muted shadow-[0_0_0_1px_rgba(15,23,42,0.10)]"
            />
            <p className="inline-flex items-center rounded-full border border-border bg-background px-3 py-1 font-mono text-[11px] font-medium tabular-nums text-muted-foreground">
              {entry.date}
            </p>
            <div className="mt-3 rounded-[1.5rem] border border-border bg-background p-6 shadow-[0_8px_40px_rgba(15,23,42,0.05)] md:p-7">
