...
def test_image_posts_and_providers_routes_require_auth(monkeypatch):
    ...
    assert client.post("/api/v1/jev/auto-mode", json={"tool": "bash", "state": {}}).status_code == 401
    assert client.post("/api/v1/jev/next-action", json={"state": {}, "targets": {"a": "b"}}).status_code == 401
    assert client.post("/api/v1/jev/screen", json={"state": {}}).status_code == 401
    assert client.post("/api/v1/jev/ads/judge", json={"state": {}}).status_code == 401
    assert client.post("/api/v1/jev/symbolic/foreman", json={"state": {}}).status_code == 401
