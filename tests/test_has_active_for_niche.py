...
has_active_for_niche is what nightly_batch uses to skip a niche that
already has work in flight. It used to reuse the reaper status list
(which deliberately omits awaiting_approval) and a 45-minute created_at
window — so a parked approve_before_post job vanished from the guard
long before the next posting window, and the cron spawned another full
render.
"""
...
async def test_has_active_sql_blocks_awaiting_approval_without_age_window(monkeypatch):
    ...
    # Age window applies only to in-flight/reapable statuses, not parking.
