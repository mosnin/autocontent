...
- **HTTP bounds** — 40/min Jev judges, 20/min ads (judge + connect/budget/status/decide/refresh/create/governance/disconnect) + article social, 10/min enqueue/retry/approve/reject/replay/remix + library compositions + campaign create/start/pause/items + kits create + template create + admin mutations + brand-kit PUT + webhook create/test + x402 credits, 8/min voice + niche draft/create/update, 5/min billing checkout + account erase. Every `/jev` POST meters spend fail-open. Video QA + Foreman + repurpose is one fan-out. Character sheet overlaps ideation + script. VO overlaps music. Article email + webhook gather. Image archive overlaps Auto Mode. Duration / empty captions fail without a Jev hop. Script and image-post fact lock strip invented stats. Composio reuses one client.
