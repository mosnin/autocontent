...
- **HTTP bounds** — 40/min Jev judges, 20/min ads (judge + connect/budget/status/decide/refresh/create/governance) + article social, 10/min enqueue/retry/approve/reject/replay/remix + library compositions + campaign start/pause + webhook create/test + x402 credits, 8/min voice + niche draft, 5/min billing checkout. Every `/jev` POST meters spend fail-open. Video QA + Foreman + repurpose is one fan-out. Archive overlaps Auto Mode. Duration / empty captions fail without a Jev hop.
