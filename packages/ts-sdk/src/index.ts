...
export class MarketerClient {
  ...
  async billingBalance(): Promise<Record<string, unknown>> {
    return this.request("GET", "/api/v1/billing/balance");
  }

  async billingCheckout(pack: string): Promise<{ url: string }> {
    return this.request("POST", "/api/v1/billing/checkout", { body: { pack } });
  }
  ...
  async createCampaign(body: Record<string, unknown>): Promise<Record<string, unknown>> {
    return this.request("POST", "/api/v1/campaigns", { body });
  }

  async startCampaign(campaignId: string): Promise<Record<string, unknown>> {
    return this.request("POST", `/api/v1/campaigns/${campaignId}/start`);
  }

  async pauseCampaign(campaignId: string): Promise<Record<string, unknown>> {
    return this.request("POST", `/api/v1/campaigns/${campaignId}/pause`);
  }

  async listLibrary(opts: { kind?: string; limit?: number } = {}): Promise<unknown[]> {
    return this.request("GET", "/api/v1/library", {
      query: { kind: opts.kind, limit: opts.limit },
    });
  }

  async listFailures(): Promise<Record<string, unknown>> {
    return this.request("GET", "/api/v1/failures");
  }

  async spendHistory(opts: { days?: number; nicheId?: string } = {}): Promise<Record<string, unknown>> {
    return this.request("GET", "/api/v1/spend/history", {
      query: { days: opts.days, niche_id: opts.nicheId },
    });
  }
